...
<style>
    ...
    @if($item->is_bid_ride == false)
                            ...
                            if($requestBill['additional_charges_amount'] > 0){
                                $bill = $requestBill->only(['base_price','distance_price','time_price','waiting_charge','cancellation_fee','service_tax','promo_discount','total_amount','admin_commision','driver_commision','admin_commision_from_driver','additional_charges_amount']);
                            }else{
                                $bill = $requestBill->only(['base_price','distance_price','time_price','waiting_charge','cancellation_fee','service_tax','promo_discount','total_amount','admin_commision','driver_commision','admin_commision_from_driver']);
                            }
                            $bill->all();
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $key == 'total_amount' ? 'highlight' : '' }}">
                                    ...
                                    <td>
                                        ...
                                        @elseif ($key == 'additional_charges_amount')
                                        {{   $item->additional_charges_reason  }}
                                        @else
                                        -
                                        @endif
                                    </td>
    ...
    @else
                            ...
                            if($requestBill['additional_charges_amount'] > 0){
                                $bill->additional_charges_amount = $requestBill['additional_charges_amount'];
                            }
    ...
    <script type="text/javascript"
        src="https://maps.googleapis.com/maps/api/js?key={{ get_settings('google_map_key') }}'google_map_key')}}&sensor=false&libraries=places"></script>
...
@if($item->is_bid_ride == false)
                        ...
                        $requestBill = collect($item->requestBill->toArray());
                        $bill =
                        $requestBill->only(['base_price','distance_price','time_price','waiting_charge','cancellation_fee','service_tax','promo_discount','total_amount','admin_commision','driver_commision','additional_charges_amount','additional_charges_reason']);
                        $bill->all();

                        $bill = $bill->toArray();
                        $bill[$requestBill->additional_charges_reason] = $requestBill->additional_charges_amount;
                        @endphp
                        ...
                        <tbody>
                            ...
                            <tr class="{{ $key == 'total_amount' ? 'highlight' : '' }}">
                                ...
                                <td>
                                    ...
                                    {{  'For first ' . $item->requestBill->base_distance .' '. $item->request_unit }}
                                    @elseif ($key == 'additional_charges_amount')
                                        {{   $item->requestBill->additional_charges_reason  }}
                                    @else
                                    -
                                    @endif
                                </td>
